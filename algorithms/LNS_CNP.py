...
class ALNS(object):
    ...
    def initial_operator_manager(self):
        ...
        self.operator_weight = np.ones((len(self.destroy_list), len(self.repair_list)))
        self.operator_call_time = np.ones((len(self.destroy_list), len(self.repair_list)))

    def select_destroy_method(self):
        """
        选择破坏算子
        :return: None
        """
        # calculate the weight
        total_weight = np.sum(self.operator_weight, axis=1)

        # make sure sum is 1，regularize it
        total_weight /= np.sum(total_weight)

        # chose a destroy operator
        chosen_idx = np.random.choice(len(self.destroy_list), 1, p=total_weight)[0]

        return self.destroy_list[chosen_idx], chosen_idx
    ...
    def select_repair_method(self):
        ...
        """
        # calculate the weight
        total_weight = np.sum(self.operator_weight, axis=0)

        # make sure sum is 1，regularize it
        total_weight /= np.sum(total_weight)

        # chose a repair operator
        chosen_idx = np.random.choice(len(self.repair_list), 1, p=total_weight)[0]

        return self.repair_list[chosen_idx], chosen_idx
    ...
    def run(self):
        ...
        for _ in trange(self.max_iterations, desc="Iteration epoch"):
            ...
            # 2. select operator
            destroy_method, destroy_index = self.select_destroy_method()
            repair_method, repair_index = self.select_repair_method()

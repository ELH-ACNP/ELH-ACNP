...
class LLF(object):
    ...
    def construct_initial_solution(self):
        ...
        :return: None
        """
        laxity = self.tasklist[:, 3] * self.tasklist[:, 2] / ((self.tasklist[:, 1] - self.tasklist[:, 0])
                                                              * self.bandwidth)
        sorted_index = laxity.argsort()
    ...
    def run(self):
        ...
        print("LLF-CNP benefit:", self.benefit)
...
def execute_llf_cnp():
    ...
    algorithm = LLF(config_dir=config_file_name)
    algorithm.run()

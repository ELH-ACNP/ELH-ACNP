...
class ALNS(object):
    ...
    def initial_task_world(self):
        ...
        # 随机生成器
        algorithm_seed = int(os.environ.get("SATELLITE_ALGORITHM_SEED", "77"))
    ...
    def initial_operator_manager(self):
        ...
        try:
            with open('../data/weight_trained.pkl', 'rb') as f:
                self.operator_weight = pickle.load(f)
        except Exception:
            print("Pretrained weight loading failed.")
    ...
    def run(self):
        ...
        for _ in trange(self.max_iterations, desc="Iteration epoch"):
            ...
            self.bid_time += (end_time1 - start_time1)

            # 3. execute the operator
            destroy_method()

            start_time1 = time.time()
            self.disposal_time += (start_time1 - end_time1)
            repair_method()
    ...
    def plot_schedule(self):
        ...
        fig_schedule = plt.figure(figsize=(8, 6))
        # fig_schedule.suptitle("Spectrum Resource Usage", fontsize=6, x=0.5, y=0.95, ha='center', va='top')
        fig_schedule.text(0.06, 0.5, '带宽(MHz)', fontproperties=font, va='center', rotation='vertical', fontsize=14)
        for satellite in range(self.satellite_num):
            ax = plt.subplot(self.satellite_num, 1, satellite + 1)
            ax.set_title("卫星 " + str(satellite+1), fontproperties=font, fontsize=14, pad=3, color='#0096A6')
            if satellite == (self.satellite_num - 1):
                ax.set_xlabel("时间", fontproperties=font, fontsize=14)
            ax.set_xlim([0, 0.5])  # 时间范围
            ax.set_ylim([0, 22])
            ax.tick_params(axis='both', which='major', labelsize=14)
            ax.set_xticks(ticks=[round(i, 1) for i in np.arange(0, 0.6, 0.1)])
            if satellite == 4:
                ax.set_xticklabels(["0:00", "4:48", "9:36", "14:24", "19:12", "24:00"], fontsize=14)
            else:
                ax.set_xticklabels([])
            # 设置颜色
            ...
            for time, width_change in events:
                if time > last_event_time:
                    ...
                    rect = patches.Rectangle((last_event_time, 0), time - last_event_time, curr_width,
                                             facecolor='#0096A6', alpha=0.8)
                    ax.add_patch(rect)
                    # 绘图
                last_event_time = time
                curr_width += width_change

        fig_schedule.subplots_adjust(hspace=1)
...
def execute_pre_tuned_alns_cnp(
        gama,
        _max_not_improve=5,
        _time_segments_iter=1,
        _return_components=False,
        _return_bcr_curve=False,
):
    """
    execute function
    :return:
    ...
    if _return_components:
        return algorithm.time, algorithm.bid_time, algorithm.disposal_time
    if _return_bcr_curve:
        total_benefit = np.sum(algorithm.tasklist[:, 4])
        return (np.asarray(algorithm.benefit_curve) / total_benefit).tolist()
    if _max_not_improve == 5:
        return algorithm.time, algorithm.benefit_curve[-1], np.sum(algorithm.tasklist[:, 4])
    else:
        return algorithm.benefit_curve
...
if __name__ == "__main__":
    ...
    Algorithm = ALNS(config_dir=Config_file_name, max_iterations=Max_iterations, max_not_improve=Max_not_improve,
                     temperature=Temperature, time_segments_iter=Time_segments_iter,

...
class Painter(object):
    ...
    def draw_benefit_mix_mean_error_graph(self):
        ...
        ax.set_xlabel('任务数量', fontproperties=font, fontsize=14, labelpad=2)
        ax.set_ylabel('收益完成率', fontproperties=font, fontsize=14, labelpad=2)
        ax.legend(fontsize=14, ncol=2)
        plt.savefig("diff_task_num_bcr.pdf")
        plt.show()
    ...
    def draw_runtime_mix_mean_error_graph(self):
        ...
        for key in self.data_reshaped:
            ...
            ax.fill_between(x_values, y_low, y_high,
                            color=self.color_palette[key],
                            alpha=0.35)
        ax.tick_params(axis='both', which='major', labelsize=14)
        ax.set_xlabel('任务数量', fontproperties=font, fontsize=14, labelpad=2)
        ...
        ax.legend(fontsize=14, ncol=2)
        plt.tight_layout()
        plt.savefig("diff_task_num_runtime.pdf")
        plt.show()
